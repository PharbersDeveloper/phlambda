...
import { fromRFC6570, UrlTemplate } from "./types/UrlTemplate"
...
export interface InputURLTemplates { [linkName: string]: UrlTemplate | string }
...
export interface InputErrorsConfig {
...
export interface ResourceTypeDescriptions {
...
export default class ResourceTypeRegistry {
  ...
  public type(typeName: string) {
    return Maybe(this._types[typeName])
      .map((it) => it.toJS() as OutputResourceTypeDescription)
      .getOrDefault(undefined)
  }

  public hasType(typeName: string) {
    return typeName in this._types
  }

  public urlTemplates(): UrlTemplatesByType
  public urlTemplates(type: string): UrlTemplates
  public urlTemplates(type?: string) {
    if (type) {
      return Maybe(this._types[type])
        .map((it) => it.get("urlTemplates"))
        .map((it) => it.toJS() as UrlTemplates)
        .getOrDefault(undefined)
    }
  ...
  public dbAdapter(typeName: string) {
    return this.doGet("dbAdapter", typeName)
  }

  public uniqueAdapters() {
  ...
  public beforeSave(typeName: string) {
    return this.doGet("beforeSave", typeName)
  }

  public beforeRender(typeName: string) {
    return this.doGet("beforeRender", typeName)
  }

  public defaultIncludes(typeName: string) {
    return this.doGet("defaultIncludes", typeName)
  }

  public info(typeName: string) {
    return this.doGet("info", typeName)
  }

  public transformLinkage(typeName: string) {
    return this.doGet("transformLinkage", typeName) as boolean
  }

  public parentTypeName(typeName: string) {
    return this.doGet("parentType", typeName)
  }

  public pagination(typeName: string) {
   ...
   */
  public asTypePath(typesList: string[], throughType?: string): false | string[] {
