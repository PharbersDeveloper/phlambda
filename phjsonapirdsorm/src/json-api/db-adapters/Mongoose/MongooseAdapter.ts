...
import { AndExpression, ExpressionSort, SupportedOperators } from "../../types/"
...
import { partition, reduceToObject, setDifference } from "../../util/misc"
import { getTypeName } from "../../util/naming-conventions"
import { values as objectValues } from "../../util/objectValueEntries"
import { Adapter, ReturnedResource, TypeIdMapOf, TypeInfo } from "../AdapterInterface"
import * as util from "./lib"
import docToResource from "./utils/doc-to-resource"
import {
    getDiscriminatorKey, getMetaKeys,
    getReferencedModelName, getReferencePaths, getVersionKey
} from "./utils/schema"
...
export default class MongooseAdapter implements Adapter<typeof MongooseAdapter> {
    ...
    public static supportedOperators: SupportedOperators = {
     ...
     * @param {object} fields
     */
    public static docsToResourceData(
        models: any,
    ...
    public static getStandardizedSchema(
    ...
    public static toFriendlyName(pathOrModelName: string) {
     ...
     */
    public static assertIdsValid(filters: AndExpression, isSingular: boolean): void {
    ...
    public docToResource(doc: Document, fields?: object) {
        return docToResource(this.models, this.modelNamesToTypeNames, doc, fields)
    }

    public docsToResourceData(docs: null | Document | Document[], isPlural: boolean, fields?: object) {
    ...
    public getTypePath(model: Model<any>) {
     ...
     */
    public async create(query: CreateQuery) {
    ...
    /**
     */
    public async update(query: UpdateQuery) {
            ...
            updated: await singleDocUpdateQueries.flatMapAsync(async ([docUpdateQuery]) => {
                const doc: mongoose.Document =
                    await (docUpdateQuery.exec().catch((e) => {
                        util.errorHandler(e, {
                            type: this.modelNamesToTypeNames[(docUpdateQuery as any).model.modelName] as string,
    ...
    public async delete(query: DeleteQuery) {
        ...
        const BaseModel = this.getModel(this.modelNamesToTypeNames[baseModelName] as string)
     ...
     */
    public async addToRelationship(query: AddToRelationshipQuery) {
     ...
     */
    public async removeFromRelationship(query: RemoveFromRelationshipQuery) {
        return this.updateRelationship(query)
    }
     ...
     */
    public async getTypePaths(items: Array<{ type: string; id: string }>) {
        ...
        for (const type of types) {
            ...
            const modelForLeanDoc = (it: any) =>
                (discriminatorKey && it[discriminatorKey])
                    ? this.getModel(this.modelNamesToTypeNames[it[discriminatorKey]] as string)
                    : BaseModel
    ...
    public getModel(typeName: string) {
     ...
     */
    public getRelationshipNames(typeName) {
