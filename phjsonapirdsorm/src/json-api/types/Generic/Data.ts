import { AsyncMapper, DataOf, Mapper, PredicateFn, Reducer } from "../index"
 ...
 */
export interface INTERNAL_DONT_USE<T> {
  data: T[]
  isSingular: boolean
}
...
export default class Data<T> {
   ...
   * @type {[type]}
   */
  public static empty: Data<any> = new Data<any>({ data: [], isSingular: true })

  /**
   * A function that lifts a value parsed from JSON:API JSON into the monad.
   */
  public static fromJSON<T>(data: DataOf<T>): Data<T> {
    return data === null
      ? this.empty
  ...
  public flatMap<U>(fn: (it: T) => Data<U>) {
    return this.flatMapHelper(this.value.data.map(fn))
  }

  public async flatMapAsync<U>(fn: (it: T) => Data<U> | Promise<Data<U>>) {
    return this.flatMapHelper(await Promise.all(this.value.data.map(fn)))
  }

  public map<U>(fn: Mapper<T, U>) {
    return this.mapHelper(this.value.data.map(fn))
  }

  public async mapAsync<U>(fn: AsyncMapper<T, U>) {
    return this.mapHelper(await Promise.all(this.value.data.map(fn)))
  }

  public every(fn: PredicateFn<T>) {
    return this.value.data.every(fn)
  }

  public some(fn: PredicateFn<T>) {
    return this.value.data.some(fn)
  }

  public forEach(fn: (it: T) => void) {
    this.value.data.forEach(fn)
    return this // for chaining
  }

  public reduce(fn: Reducer<T, T>): T | undefined
  public reduce<U>(fn: Reducer<T, U>, initialValue: U): U
  public reduce<U>(fn: Reducer<T, U>, initialValue?: U): U | T | undefined {
  ...
  public filter(fn: PredicateFn<T>) {
   ...
   */
  private flatMapHelper<U>(newDatas: Array<Data<U>>): Data<U> {
