...
import {
  AsyncMapper, Links, Mapper, PredicateFn, Reducer, UrlTemplates
} from "./index"
 ...
 */
export interface MaybeDataWithLinksArgs<T> {
...
export default class MaybeDataWithLinks<T extends (Resource | ResourceIdentifier)> {
  ...
  public map(fn: Mapper<T, T>) {
    return this.delegateTransformToData("map", arguments)
  }

  public flatMap(fn: (it: T) => Data<T>) {
  ...
  public mapAsync(fn: AsyncMapper<T, T>) {
    return this.delegateTransformToDataAsync("mapAsync", arguments)
  }

  public flatMapAsync(fn: (it: T) => Data<T> | Promise<Data<T>>) {
   ...
   */
  public unwrapWith<U>(fn: (it: T) => U, linkTemplateData: any) {
    return {
      links: mapObject(
        this.links || {},
   ...
   */
  public unwrapDataWith<U>(fn: (it: T) => U) {
    return this._data && this._data.map(fn).unwrap()
  }

  public every(fn: PredicateFn<T>): boolean {
    return this._data ? this._data.every(fn) : true
  }

  public some(fn: PredicateFn<T>): boolean {
