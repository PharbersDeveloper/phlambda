...
import { AWSRequest, dbFactory, redis, store } from "phnodelayer"
import {
    errors2response,
    PhInvalidAuthGrant,
    PhInvalidClient,
...
export default class AppLambdaDelegate {
    ...
    protected async genAuthCode(uid: string, cid: string, scope: string) {
        ...
        await this.rds.setExpire(`authorization:${result.payload.records[0].id}`, JSON.stringify(result.payload.records[0]), seconds.toFixed(0))
        return code
    }
    ...
    protected async genAccessToken(uid: string, cid: string, scope: string) {
        ...
        // tslint:disable-next-line:max-line-length
        await this.rds.setExpire(`access:${result.payload.records[0].id}`, JSON.stringify(result.payload.records[0]), seconds.toFixed(0))
        return tk
    }
