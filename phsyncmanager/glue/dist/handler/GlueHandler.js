...
const AWSGlue_1 = __importDefault(require("../utils/AWSGlue"));
class GlueHandler {
    ...
    async syncAll(updatePartitionCount = false) {
        const instance = new AWSGlue_1.default(this.config);
        const client = instance.getClient();
        const pageDataBaseContent = await client_glue_1.paginateGetDatabases({ client }, {});
        const databaseNames = [];
        const tableNames = [];
        ...
        for (const dbName of databaseNames) {
            const pageTableContent = await client_glue_1.paginateGetTables({ client }, { DatabaseName: dbName });
    ...
    async addPartitionCountToTableAttr(databaseName, tableName) {
        const instance = new AWSGlue_1.default(this.config);
        const client = instance.getClient();
        const partitions = await client_glue_1.paginateGetPartitions({ client }, {
        ...
        const getTableCommand = new client_glue_1.GetTableCommand({
            DatabaseName: databaseName,
            Name: tableName
        });
        ...
        const updateCommand = new client_glue_1.UpdateTableCommand({
            DatabaseName: databaseName,
            TableInput: tableInput
        });
        await client.send(updateCommand);
    }
    async syncDatabases(databaseName, action) {
        const instance = new AWSGlue_1.default(this.config);
        const client = instance.getClient();
        const command = new client_glue_1.GetDatabaseCommand({
            Name: databaseName
        });
    ...
    async syncTables(databaseName, tableName, action) {
        const instance = new AWSGlue_1.default(this.config);
        const client = instance.getClient();
        const pageContent = await client_glue_1.paginateGetTableVersions({ client }, {
            DatabaseName: databaseName,
            TableName: tableName
        });
...
exports.default = GlueHandler;
//# sourceMappingURL=GlueHandler.js.map