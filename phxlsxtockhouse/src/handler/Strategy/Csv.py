...
#         print(data)
#         print(str(data[0][0]))
#         # print(0 if str(data[0][0]) == "nan" else data[0][0])
#         return list(map(lambda x: dict(zip(self.schema+["version"], x)), [[str(j) if str(j) != "nan" else "None" for j in i + [version]] for i in data]))
#
...
class Csv:
    ...
    def do_exec(self, data):
        ...
        filename1 = parameters.get("file_name")
        filename = parameters.get("file_name").split(".")[0]
        temp_file = os.environ.get("")
        ds_name = parameters.get("ds_name")
        projectId = parameters.get("projectId")
        path = f"/Users/moke/worker_space/phlambda/phxlsxtockhouse/src/handler/Strategy/{filename1}"
        ...
        for i, dataf in enumerate(data_list):
            ...
            if not self.schema:
                ...
                new_data = self.parse_data(data_l, version)
                self.toclickhouse(table_name, new_data)
            # out_file_name = f"/mnt/tmp/{projectId}/tmp/" + filename + "_" + str(i + 1) + ".parquet"
