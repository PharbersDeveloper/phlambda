...
class GenCronExpression:
    ...
    def get_cron_expression(self):
        ...
        result = list(map(lambda x: x[-1] if map_TimeAndValue.index(x) < indexOfPeriod else (f"*/{self.period_value}" if map_TimeAndValue.index(x) == indexOfPeriod else x[-1]), map_TimeAndValue))
