...
def SendEmail(Result, ToNickName, ToEmail):
    try:
        mail_msg = getResultHTML(Result)
        msg = MIMEText(mail_msg, 'html', 'utf-8')
        msg['From'] = formataddr((sendNickName, my_sender))  # 括号里的对应发件人邮箱昵称、发件人邮箱账号
        msg['To'] = ','.join(ToEmail)    #formataddr((ToNickName, ToEmail))              # 括号里的对应收件人邮箱昵称、收件人邮箱账号
        ...
        server.sendmail(my_sender, ToEmail, msg.as_string())  # 括号中对应的是发件人邮箱账号、收件人邮箱账号、发送邮件
        #server.sendmail(my_sender, [ToEmail, ], msg.as_string())  # 括号中对应的是发件人邮箱账号、收件人邮箱账号、发送邮件
        server.quit()  # 关闭连接
        print("邮件发送成功")
