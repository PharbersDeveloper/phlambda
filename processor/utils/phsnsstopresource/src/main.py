...
def lambda_handler(event, context):
    ...
    tenantId = ''
    for dimensions in [metricalarm.get("Dimensions") for metricalarm in response.get("MetricAlarms") if
                       metricalarm.get("StateValue") == "ALARM"]:
    ...
    if tenantId:
        ...
        StopJupyter().run(tenantId, ctype="c9")
    return True
